...
async function start() {
  ...
  try {
    ...
    for (const verse of verses) {
      ...
      if (verse.audio && verse.audio.startsWith("http")) {
        ...
        try {
          ...
          audioFileName = `temp-render/${audioFile}`; // relative to public/
          ...
          console.warn(`  ⚠ Could not download audio for verse ${verse.id}: ${e.message}`);
          totalDurationSeconds += 8; // fallback
        }
      } else {
        totalDurationSeconds += 8; // no audio, use default
      }
    ...
    // Add 1 second padding between verses
    totalDurationSeconds += (processedVerses.length - 1) * 0.5;
    // Minimum 5 seconds
    totalDurationSeconds = Math.max(5, totalDurationSeconds);
    ...
    console.log(`>> All ${processedVerses.length} audio files downloaded ✓`);
    ...
    const comps = await getCompositions(bundleLocation, {
      inputProps,
      staticDir: publicDir, // CRITICAL: tells Remotion where static assets live
    });
    ...
    console.log(">> Rendering video (parallel processing)...");
    await renderMedia({
      composition,
